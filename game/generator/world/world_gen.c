	...
	for (i64 z = 0; z < ST_GEN_CHUNK_SIZE; z++) {
		for (i64 x = 0; x < ST_GEN_CHUNK_SIZE; x++) {
			...
			map[idx] = (StTerrainPart){
				.height = ST_GEN_BASE_HEIGHT + (height * 20) + 20,
...
// 	meshma.vertexCount = ST_GEN_CHUNK_SIZE * ST_GEN_CHUNK_SIZE;
...
// 	for (i64 z = 0; z < ST_GEN_CHUNK_SIZE - 1; z++) {
// 		for (i64 x = 0; x < ST_GEN_CHUNK_SIZE - 1; x++) {
