...
public static class Graphics {
    ...
    public static void recompileShaders()
    {
        Window.actions.Enqueue(async () => {
            ...
            await mainShader.compile();
            await shadowShader.compile();
            await debugShader.compile();
            await skyboxShader.compile();
        });
        Window.actionLoopEvent.Set();
    }
    ...
    /// </summary>
    static Task<aabb> addAabb(Object3D obj) {
        TaskCompletionSource<aabb> tcs = new();
        Window.actions.Enqueue(() => {
            if (gl == null) {
                tcs.SetResult(obj.box);
                return;
            }
